...
### 修复（M2 实操游玩发现）
- **AI「眼球团/重排鱼群」单张浅滩冻结**：AI 在仅 1 张牌的浅滩发动重排时，游戏卡在待确认弹窗。
  - `heuristicAI` 只对 >1 张的浅滩选重排；REARRANGE pending 若仅 1 张直接自动结算；`main.js` 新增 `drainAiPending` 让 M2 回合自动结算轮到 AI 的一切反应窗口（REDIRECT/COUNTER/REARRANGE/PASS_LEFT），并从根杜绝 AI 产生 pending 后冻结。

### 变更
- **空浅滩放回视觉**：空浅滩作为合法放回目标时，整堆显示磷光绿描边 + 径向背景 + 脉冲动画 + 底部「放回」角标，新手不再困惑空堆可点。
- **AI 额外抽牌更积极**：`chooseAbilityAction` 优先发动皇带鱼/七鳃鳗/海神之怒（额外抽牌/加力量），先锁定本回合抽牌上限与力量再处理其它能力。
- 跳过能力阶段按钮文案 `跳过能力阶段 →` → `跳过能力阶段（不发动）`（去歧义、保留阶段名）。
- **放回非合法浅滩不再静默**：点非法（非高亮）浅滩会 toast「只能放回高亮（可操作）的浅滩」，无可放回目标时提示「当前回合不允许放回这张牌」。

### 测试
- 新增 `tests/defense/aiRearrangeFreeze.test.js`（单张不选重排 / 多张仍选 / 额外抽牌优先）、空浅滩放回角标渲染断言，以及 `tests/ui/throwToast.test.js`（非法放回 toast）；**178 测试全绿**、压测 60 局 2p + 4 局 4p 无死锁、`npm run build` 通过。

## [0.8.1] - 2026-08-29

### 修复（深度随机压测发现的横置态一致性 Bug）
- 梭子鱼移除对方已钓的难度 0 鱼时，同步清除该卡在该玩家 `exhausted` 中的记录，不再残留指向不存在的卡牌。
- 断脚送出卡时，横置态随卡转移到新主人（与原主解除、目标追加），新主人无法重复发动，且原主不再残留 stale 横置记录。
- 状态机 `USE_ABILITY` 仅在使用的主动卡仍属当前玩家 `caught` 时才写回横置，避免"已交出的卡"残留横置在原主。
- 新增 `tools/stress.mjs` 深核压测 harness（驱动真实 AI 跑 2/4 人对局，多种子校验死锁/僵局/状态不变量）；新增 2 个确定性回归单测。
